...
def inference_attention_maps(fp_experiment, fp_out, n_images=4):
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm.tqdm(enumerate(test_loader), 
            ...
            # We'll plot the original, and one vis for every head
            n_head_half = math.ceil( n_heads // 2 )
            fig, axs = plt.subplots(3, n_head_half) #gridspec_kw={'height_ratios': [3, 1]})
            fig.set_size_inches(20, 20)
            [axi.set_axis_off() for axi in axs.ravel()]

            # Make a visualisation for every head
            for j in range(n_heads):
                ...
                # Visualise each head
                attn_map_scaled = attn_map_scaled[0,j,:,:] # (~W, ~H)
                ...
                axs[1 if j < n_head_half else 2, j % n_head_half].imshow(attn_map_scaled, cmap="twilight")
                #plt.imsave(
